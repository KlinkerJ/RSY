function theta = inverse_kinematics(obj, pos_new, eul_new)
    ...
    T06(1:3, 1:3) = eul2rotm(eul_new, 'XYZ'); % write rotations in the upper left corner of the matrix
    T06(1:3, 4) = pos_new; % write position on the right side of the matrix
    P06 = T06(1:3, 4); % origin of frame 6
    % ------------------------------ Theta 1 ------------------------------
    % Theta1 = 2 solutions, isn't depended on other angles.

    P05 = T06 * [0; 0; -obj.d(6); 1]; % origin of frame 5 (formula 5 [1])
    ...
    if P05(1) ~= 0 || P05(2) ~= 0 % unequality because square root would fail if both are zero (zero under square root)
        phi2P = acos(obj.d(4) / sqrt(P05(1)^2 + P05(2)^2)); % (formula 8 [1])
    ...
    for i = 1:length(theta1)
        acosValue = (P06(1) * sin(theta1(i)) - P06(2) * cos(theta1(i)) - obj.d(4)) / obj.d(6); % (formula 12 [1])
    ...
    for t1 = 1:length(theta1)
        ...
        for sign = [1 -1]
            [theta3_buff, P14, T14] = calculateTheta3(obj, T06, theta1(t1), theta5(t5(t1)), theta6(t5(t1))); % (part of formula 19 [1])
            theta3(idx) = sign * theta3_buff; % (part of formula 19 [1])
            P14_(idx, :) = P14';
            T14_(:, :, idx) = T14;
            idx = idx + 1;

            [theta3_buff, P14, T14] = calculateTheta3(obj, T06, theta1(t1), theta5(t5(t1) + 1), theta6(t5(t1) + 1)); % (part of formula 19 [1])
            theta3(idx) = sign * theta3_buff; % (part of formula 19 [1])
            P14_(idx, :) = P14';
            T14_(:, :, idx) = T14;
            idx = idx + 1;
    ...
    for i = 1:length(theta3)
        ...
        theta2(i) = atan2(-P14_(i, 3), -P14_(i, 1)) - asin(-obj.a(3) * sin(theta3(i)) / P14_xz_length); % (formula 22 [1])
    end
    ...
    for t2t3 = 1:length(theta2)
        T12 = DH2tform(obj.alphaArr(1), obj.a(1), obj.d(2), theta2(t2t3));
        T23 = DH2tform(obj.alphaArr(2), obj.a(2), obj.d(3), theta3(t2t3));
    ...
    solution = closetSolution(solutions, obj.theta);
    theta = solution';
end
...
function [theta3_buff, P14, T14] = calculateTheta3(obj, T06, theta1, theta5, theta6)
    [P14, T14] = calculateP14(obj, T06, theta1, theta5, theta6);

    P14_xz_length = norm([P14(1) P14(3)]);

    conditions = [abs(obj.a(2) - obj.a(3)); abs(obj.a(2) + obj.a(3))]; % (conditions described after formula 19 [1])

    if P14_xz_length > conditions(1) && P14_xz_length < conditions(2) % (conditions described after formula 19 [1])
        theta3_buff = acos((P14_xz_length^2 - obj.a(2)^2 -obj.a(3)^2)/(2*obj.a(2)*obj.a(3))); % (formula 19 [1])
...
function [P14, T14] = calculateP14(obj, T06, theta1, theta5, theta6)
    T01 = DH2tform(0, 0, obj.d(1), theta1);
    T45 = DH2tform(obj.alphaArr(4), obj.a(4), obj.d(5), theta5);
    T56 = DH2tform(obj.alphaArr(5), obj.a(5), obj.d(6), theta6);
